...
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii38-2026-07-29-1830.ps1
# BUILD:   ascii38  |  Version 3.1
...
function Show-LookBack {
    ...
    while ($true) {
        if ($ggIdx -le 0) {
            Clear-Host
            Write-Host ""
            Write-Host "  This is as far back as the tool can show you." -ForegroundColor Yellow
            Write-Host "  Everything from this run is in your log file:" -ForegroundColor Gray
            ...
            if (-not (Restore-ScreenSnapshot -Snapshot $script:GGSnapshots[$ggIdx])) {
                Clear-Host
                Write-Host ""
                Write-Host "  That earlier screen cannot be shown again -- the window size" -ForegroundColor Yellow
                Write-Host "  changed since it was displayed. It is in your log file:" -ForegroundColor Yellow
                Write-Host "    $LogPath" -ForegroundColor Gray
            }
        }
        Write-Host ""
...
function Show-FontInstructions {
    ...
    $introScreens = @(
        {   # Screen 1 of 5: Welcome + maximize (FT-10 final wording)
            Write-Host "  Welcome to GatewayGuard Checkup.  (Screen 1 of 5)" -ForegroundColor Cyan
            Write-Host ""
...
function Show-PreScanGate {
    ...
    if ($global:IsFirstRun) {
        ...
        Draw-Box -ScreenId "39" -Color White -Lines @(
            ...
            "  * Malwarebytes scan (Checkup can launch this for you)      ",
...
Clear-Host
Write-Host ""
Write-Host "  GatewayGuard Checkup -- Windows 11 Security Hardening v$ScriptVersion -- $BuildID" -ForegroundColor DarkGray
