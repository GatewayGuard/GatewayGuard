# Dated: 2026-07-27 16:55 EDT
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii36-2026-07-27-1655.ps1
# BUILD:   ascii36  |  Version 3.1
...
function Show-ScanPlanBriefing {
    ...
    Write-Log -Message "[SCREEN-27] Rendered: The Scans We Recommend (briefing 2 of 2)" -Status "SCREEN"
    Draw-Box -Color White -Lines @(
...
function Show-MalwarebytesFollowUp {
    ...
    if ($mbState -eq "NotInstalled") {
        Write-Log -Message "[SCREEN-13] Rendered: Malwarebytes Guidance (not detected)" -Status "SCREEN"
        Draw-Box -Color White -Lines @(
        ...
        Write-Log -Message "[SCREEN-13] Rendered: Malwarebytes Guidance (Custom Scan rewrite)" -Status "SCREEN"
        Draw-Box -Color White -Lines @(
...
function Apply-PowerSettings {
    ...
    if ($Choices["WakeOnLAN"]) {
        try {
            ...
            if ($failed -gt 0) {
                Write-Host "  WARN Wake on LAN -- $changed changed, $failed could NOT be changed" -ForegroundColor Yellow
                Write-Host "       Takes effect after your next restart." -ForegroundColor Gray
                Write-Log -Message "Wake on LAN partial: $changed disabled, $failed failed" -Status "APPLIED"
            } elseif ($changed -gt 0) {
                Write-Host "  OK  Wake on LAN -- disabled ($changed setting(s))" -ForegroundColor Green
                Write-Host "       Takes effect after your next restart." -ForegroundColor Gray
                Write-Log -Message "Wake on LAN disabled ($changed settings)" -Status "APPLIED"
            } else {
...
function Show-ConvenienceReview {
    ...
    Clear-Host
    Write-Host ""
    Write-Log -Message "[SCREEN-23] Rendered: Convenience Choices (ask-before-apply)" -Status "SCREEN"
...
function Run-ConsoleMode {
    ...
    :checklistLoop while ($true) {
        ...
        $ggNameW = [Math]::Floor($ggAvailable * 0.6)
...
# before any scan screen. First run only; checkpointed so resume paths
# skip it (C-20 acknowledgement printed when skipped on resume).
