...
$wanted = @(
    ...
    @{ Label = 'Password manager research'; Pattern = 'GatewayGuard_PasswordManagerResearch-*.md' },
...
$L.Add('')
# ROWS FIRST (2026-08-31). Cloud reported four consecutive reads that
# returned ONLY the prose above the table -- header, freshness stamp and the
# "why this is here" paragraph -- and never a single row. Measured: the rows
# began at line 38, behind 37 lines of prose dense in exactly the words a
# "what is current" query matches. The file failed at its only job.
# The stamp is now four lines, the rows come first, and every word of
...
$L.Add('---')
$L.Add('')
$L.Add('## THE ROWS -- THE CURRENT, LIVE, LATEST FILENAME FOR EVERY DOCUMENT')
$L.Add('')
$L.Add('**This table is the entire purpose of this file.** Each row names one')
$L.Add('document family and the single filename that is live right now. Take every')
$L.Add('filename from this table -- never from memory, never from another document,')
$L.Add('never by guessing which date looks newest.')
$L.Add('')
$L.Add(('**' + @($resolved).Count + ' rows follow.** If you cannot see all ' + @($resolved).Count + ' of them, your'))
$L.Add('retrieval truncated the table -- **say so and ask for the rest, rather')
$L.Add('than answering from the handful you received.**')
$L.Add('')
...
$L.Add('')
$L.Add('## THE STAMP -- how to tell how old this copy is')
$L.Add('')
$L.Add('The four values at the top are the freshness stamp.')
$L.Add('')
$L.Add('**If you are reading a synced copy rather than the live repository --')
$L.Add('Claude Cloud always is -- state those values in your first reply.**')
...
$L.Add('')
$L.Add('**This filename never changes**, so it can be named in an instruction')
$L.Add('without going stale. The filenames in the table change constantly.')
$L.Add('')
