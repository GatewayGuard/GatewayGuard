...
- [ ] **23b. FT-167 -- DOES THE 1 TB D: DRIVE GET ENCRYPTED TOO?** Once
      encryption has finished, run `Tool\Run-EncryptionMeasure.bat` once more
      and note **every** volume it lists, not just C:.
      **This answers a question the project has never been able to answer.**
      Microsoft documents Device Encryption as covering the OS drive *and*
      fixed data drives, but no machine here has ever been watched through a
      Home encryption with a second fixed drive attached.
      **If D: shows encrypted:** Home covers both, and FT-167 is a Pro-only
      defect (Checkup calls `Enable-BitLocker` on C: only).
      **If D: shows unencrypted:** SANDY has 1 TB in the clear while Checkup
      reports `ENCRYPTED -- GOOD`, and FT-167 becomes a blocker for both
      editions.
...
%/min, and GB/min against both used space and total volume. Plus the
volume list from step 23b.

---

## PHASE 4b -- FT-168 FREE-SPACE WIPE TEST (SANDY D: ONLY, after encryption)

**Only on SANDY's D:, the 1 TB hard drive. NEVER on CGDELL.** CGDELL is an
NVMe SSD, and free-space wiping on solid state burns write cycles for little
benefit -- TRIM and wear-levelling mean the old cells cannot reliably be
overwritten anyway.

Run these in an **administrator** Command Prompt, one at a time.

- [ ] **25. Does the command exist and run on Home at all?** This is the
      blocker question -- `manage-bde -w` is documented mainly as a
      Pro/Enterprise tool and its Home support is unconfirmed.

      Type: `manage-bde -w D: -?`
      **PASS:** it prints usage text.
      **FAIL:** "not recognised", access denied, or an edition complaint.
      **If it fails, stop here** -- the fallback is `cipher /w:D:`, which is a
      plain Windows utility with no BitLocker dependency. Test that instead
      and record which one worked.

- [ ] **26. Note the time before you start.** This writes across all free
      space -- roughly **852 GB** on D:. Expect it to take about as long as
      the encryption itself. **Start it when you do not need the machine.**

- [ ] **27. Run it.** `manage-bde -w D:`
      Record: start time, finish time, and whether it could be interrupted
      and resumed (`manage-bde -pause` / `-resume` claim to support free
      space wipe).

- [ ] **28. Confirm the state afterwards.** Run
      `Tool\Run-EncryptionMeasure.bat` again and record D:.
      **What we are proving:** Microsoft states this leaves a used-space-only
      volume with *"the same level of protection as if the volume had been
      encrypted with the full encryption option."*

**Send back:** the times, the exact command that worked, and the final
volume state. This decides whether the free-space tip ships as a Windows
built-in, or not at all.
