...
- **Change History Log:**
  ...
  - 2026-08-12 15:12: **Section 5a pointer updated again, for the
    ...
    `START-HERE.txt`, then `Check-Claude-Cloud.txt`, now
    `Start-Claude-Cloud.txt`.
    **Also records why Cloud could not find the 2026-08-11 documents.** Not a
    missing commit: all four were tracked and on `origin/main`, with
    `git rev-list --left-right --count origin/main...main` returning `0 0`.
    **`Start-Claude-Cloud.txt` tells Cloud to glob
    `_READ-FIRST-Briefing-*.md` and take the newest by filename date, and
    Cloud cannot list or sort a directory.** With five briefings in
    `ProjectDocs/` it searches by relevance and returns whichever ranks
    highest -- a coin toss it reports as success. The anti-stale-filename
    rule is correct for Claude Code and actively harmful for Cloud, and that
    asymmetry went unnoticed from the day the file was written.
    **Second suspect still open:** the A6 connector proof passed 2026-08-11 at
    14:45; those documents were committed at 16:17, after it. Settled by one
    question -- ask Cloud to quote *"SANDY converted to a local account named
    panther"*, a sentence that exists only in the 16:16 versions. Quoting it
    clears the connector and convicts the glob; failing to clears the glob and
    convicts the connector. **Do not build a fix before running that test** --
    the two causes need completely different fixes.
    **Proposed, not yet built:** `ProjectDocs/CURRENT.md`, one fixed name Cloud
    can be told, holding the current filenames, **generated by
    `Tool/Update-Current.ps1` at session end and never typed**, refusing to
    write if a glob returns zero hits. That keeps the volatile fact in exactly
    one place while giving Cloud a pointer it can actually follow.
