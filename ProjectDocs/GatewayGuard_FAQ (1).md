...
A: Yes. GatewayGuard collects no personal data whatsoever -- there is nothing to store, share, or protect under GDPR. No account is created, no data leaves your PC, and no cookies or tracking are used on our website. GatewayGuard is GDPR compliant by design.
...
A: GatewayGuard uses the format 27-Jun-2026 (day-abbreviated month-year) for all displayed dates. This format is unambiguous in every country -- there is no confusion between day and month regardless of where you live. File names use the international ISO 8601 format (YYYY-MM-DD, e.g. 2026-06-27) which sorts correctly in every language and file system.
