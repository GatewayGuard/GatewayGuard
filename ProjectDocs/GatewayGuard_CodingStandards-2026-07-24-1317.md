...
- **Last Modified:** 2026-07-24 13:17 EDT
- **Status:** Cumulative Master Document (supersedes all prior dates)
- **Change History Log:**
  - 2026-07-24 13:17: Integrated C-20 (Skips Are Acknowledged, Never
    Silent) -- the one rule from CodingStandards_Additions-2026-07-19.md
    found missing during the 07-24 addendum-vs-master audit. Added as
    C-series prose rule and as pre-build gate 24. All 15 rules
    (C-14 through C-27) from the additions file are now confirmed in
    this master; the additions file can be retired.
...
### 24. Skips Are Acknowledged, Never Silent (C-20, anchored: FT-98)
Whenever the tool bypasses content the user knows exists -- a skipped
scan, a section not applicable to their PC, a resume past completed
work -- the next screen must open with one plain line saying what was
skipped and why. Walk every path (including conditional/skipped
screens) before ship: the user-facing step counter counts rendered
screens only (1,2,3... no gaps), and every bypass is announced in
plain language. Silent skips are a defect.
...
### Skips are acknowledged, never silent (C-20, approved 2026-07-19)
The user-facing step counter never skips numbers (it counts rendered
screens only). Whenever the tool bypasses content the user knows
exists -- a skipped scan sequence, a section not applicable to their
PC, a resume jumping past completed work -- the next screen opens with
one plain line saying so and why:
  "You chose to skip the scans -- moving on to your settings."
  "Your PC doesn't use a battery, so we skipped the battery check."
The user must never wonder whether the tool jumped over something
