...
- **Change History Log:**
  ...
  - 2026-08-12 15:12: **`Check-Claude-Cloud.txt` renamed to
    ...
    `Check-Claude-Cloud.txt`, now this.
    **Two earlier references were deliberately NOT changed** and must stay as
    written: the change-history entry above dated 2026-08-11, and the entry in
    `GatewayGuard_SyncPlan-*.md` dated 2026-08-10. A change-history entry is a
    record of what happened; editing the filename inside it makes the record
    false. The same protection covers
    `GatewayGuard_ProjectInstructions-2026-08-11-1616.md:562`, which quotes the
    literal command `git ls-files "Check-Claude-Cloud.txt" "Start-CC.txt"` --
    that is the evidence that earned VERIFICATION RULE V-1, and rewriting the
    command turns the evidence into a fiction.
    **A6 is not proof for the documents committed after it.** A6 passed
    2026-08-11 at 14:45; the `-1616` governing documents were committed at
    16:17, and Cloud has since failed to find them. **A connector proof is
    valid only for the commit it ran against.** Re-prove after any commit that
    adds documents Cloud must read -- the cheap version is one question,
    recorded in the SyncPlan entry of the same date.
...
That block is the same one already proven in `Start-Claude-Cloud.txt` (repo
root, outside the connector scope -- so Cloud cannot open it to confirm the
match itself). It was named `START-HERE.txt` until 2026-08-10, then
`Check-Claude-Cloud.txt` until 2026-08-12.
