...
| 27 | Review your selections | **This is the screen Bill hit with the Back-key defect (A3).** Confirm `B` goes back here and `N` never does. This is also where Setting 6 (phishing), Setting 3 (Tamper Protection) and Setting 9 (Windows Hello) will show their manual-instruction text if Windows blocks the automatic change -- **this is the highest-value screen to watch closely today**: Tamper Protection and Windows Hello's specific instructions have never been shown to any user before today's fix (FT-263), and Setting 6's wording changed today too (FT-262). **Also watch Settings 13 (Edge Startup Boost/Background) and 14 (Widgets):** as of today (FT-123b) both can report a real On/Off instead of "Unknown -- could not check" when Checkup hasn't yet applied them itself. **Both currently read "needs attention" on CGDELL.** Item 13's value moved between two reads an hour apart today (Off at 15:11, On at 15:43); Bill confirmed by looking at the real Edge screen that Startup Boost is genuinely On right now, matching the second read exactly -- so the key is trustworthy, this is just a genuinely live setting that can change between checks, not a defect in the read. |
