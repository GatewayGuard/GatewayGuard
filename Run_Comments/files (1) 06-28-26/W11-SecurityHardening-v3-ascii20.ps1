# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii20.ps1
# BUILD:   ascii20  |  Version 3.0
...
function Run-PowerSettingsCheck {
    ...
    try {
        ...
        $results["ScreenTimeout"] = if ($acTimeout -eq 0) { "Found: NEVER -- recommend setting to 5 min (Settings -> System -> Power & sleep)" }
                                    elseif ($minutes -le 5) { "Found: ${minutes} min -- GOOD (you set this before running)" }
                                    else { "Found: ${minutes} min -- recommend reducing to 5 min (Settings -> System -> Power & sleep)" }
...
function Show-ConvenienceReview {
    ...
    foreach ($ci in $convItems) {
        ...
        do {
            $resp = Read-Host "  Your choice (Y/N)"
        } while ($resp.ToUpper() -notin @("Y","N"))
