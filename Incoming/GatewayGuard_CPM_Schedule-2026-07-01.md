...
**Revision 3 (30-Jun-2026):** Dell Latitude 5430 picked up today — T1 closed out, schedule re-run with Dell as on-hand, Day 0. CPM network chart added.
...
```mermaid
graph LR
    T2["T2: Pro setup<br/>+ Win Update<br/>1d"] --> T3["T3: Run ascii22<br/>on Dell, Hyper-V<br/>1d"]
    T3 --> T4["T4: Build ascii23<br/>Pro-specific fixes<br/>3d"]
    T4 --> T5["T5: Regression test<br/>all 3 machines<br/>2d"]
    ...
    T10 --> T12
    T12 --> T14
    T12 --> T13["T13: Beta tester<br/>program build<br/>2d — off critical path"]
    ...
    class T2,T3,T4,T5,T6,T14,T15v,T16 critical;
    class T7,T8,T9,T10,T11,T12,T13 slack;
```

**Black nodes = critical path (zero slack).** White nodes = float available — these are where schedule risk can absorb a slip without moving the launch date. T8 (screenshots) is technically white above but carries zero slack in the table, since its only float comes from T12's 1-day cushion — treat it as critical in practice.
