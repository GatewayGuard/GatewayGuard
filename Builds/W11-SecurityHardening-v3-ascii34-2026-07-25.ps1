# Dated: 2026-07-25 09:00 ET
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii34-2026-07-25.ps1
# BUILD:   ascii34  |  Version 3.1
...
# CHANGES FROM ascii33 (2026-07-25 lint pass part 2 -- PSAvoidGlobalVars):
#   Converted all 20 $global: variables to $script: scope (BatteryPct,
#   BitLockerKeyGenerated, BitLockerKeyPath, CheckpointOrder, HasBattery,
#   HasPasswordManager, IsAdmin, IsFirstRun, MachineID, MachineMake,
#   MachineModel, OnBattery, OrigScreenSaverActive, OrigScreenSaverTimeout,
#   OrigVideoIdle, RAMGB, ResumeFrom, SleepPrevented, WinEdition,
#   WinEditionFriendly). Verified none of the 20 are referenced inside
#   the 6 scope-boundary constructs in this file (the PowerShell.Exiting
#   engine event action, the console-mode trap, and the 4 WinForms
#   Add_Click/Add_FormClosing handlers) -- all reads/writes are plain
#   function-to-function, which $script: scope covers identically to
#   $global: in a single-file script. $script: also stops this tool's
#   state from leaking into the user's PowerShell session if it's ever
#   dot-sourced instead of run standalone.
#   PSAvoidUsingWriteHost (420) intentionally NOT touched -- see ascii33
#   changelog; still considered a false positive for this console app.
# CHANGES FROM ascii32 (2026-07-25 lint pass -- PSScriptAnalyzer):
#   Fixed 38 empty catch blocks (silent failures now surface via
#   Write-Verbose -- run with -Verbose to see them; default behavior
#   unchanged for end users).
#   Converted 18 Get-WmiObject calls to Get-CimInstance (deprecated
#   cmdlet warning; same underlying WMI classes/properties, no
#   behavior change).
#   Removed 2 Invoke-Expression calls in the Store/UWP app-uninstall
#   path -- now calls Remove-AppxPackage directly on PackageFullName
#   instead of building/evaling a command string.
#   Removed unused $GoodPatterns variable (dead code -- the pattern
#   was already inlined elsewhere) and the unused $Results parameter
#   on Set-PowerSetting (formerly Apply-PowerSettings).
#   Renamed WinForms FormClosing handler's $sender parameter to
#   $formSender -- PSScriptAnalyzer flagged $sender as colliding with
#   an automatic-variable-like name; kept as a required positional
#   delegate parameter, now just explicitly marked unused.
#   Renamed 13 functions to use approved verbs / singular nouns (all
#   call sites updated): Draw-Box->Show-Box, Pause-ForUser->
#   Wait-ForUser, Run-PowerSettingsCheck->Invoke-PowerSettingsCheck,
#   Apply-PowerSettings->Set-PowerSetting, Run-AppsAudit->
#   Invoke-AppsAudit, Apply-Setting->Set-SecuritySetting,
#   Setup-ScheduledTasks->Initialize-ScheduledTask, Run-ConsoleMode->
#   Start-ConsoleMode, Run-GUIMode->Start-GUIMode,
#   Show-FontInstructions->Show-FontInstruction, Get-AllStatuses->
#   Get-AllStatus, Test-NonRecommendedSelections->
#   Test-NonRecommendedSelection, Show-ManualSteps->Show-ManualStep.
#   NOT changed (by design, confirmed with dev before this pass):
#   Write-Host usage (424 flagged) and script/global variables (144
#   flagged) -- both are core to how this console tool works and were
#   scoped out of this lint pass as high-risk/low-value this close to
#   launch.
...
#   G-01:   Screen numbering: every Show-Box screen now logs [SCREEN-NN]
...
#           Start-ConsoleMode trap, which exited but could not log the ERROR
...
#   FT-49/64: Instrumentation -- every Show-Box screen renders a [SCREEN]
...
$script:BitLockerKeyGenerated = $false
$script:BitLockerKeyPath      = ""
$script:IsAdmin               = $false
$script:WinEdition            = "Unknown"
$script:IsFirstRun            = $true
$script:RAMGB                 = 0
$script:OnBattery             = $false
$script:SleepPrevented        = $false
...
$script:OrigScreenSaverActive  = $null
$script:OrigScreenSaverTimeout = $null
$script:OrigVideoIdle          = $null
function Suspend-ScreenSaver {
    try {
        $desk = "HKCU:\Control Panel\Desktop"
        $script:OrigScreenSaverActive  = (Get-ItemProperty $desk -EA SilentlyContinue).ScreenSaveActive
        $script:OrigScreenSaverTimeout = (Get-ItemProperty $desk -EA SilentlyContinue).ScreenSaveTimeOut
        ...
        $script:OrigVideoIdle = (powercfg /query SCHEME_CURRENT SUB_VIDEO VIDEOIDLE 2>$null) -match "Current AC.*0x(\w+)" | Out-Null
...
function Restore-ScreenSaver {
    try {
        $desk = "HKCU:\Control Panel\Desktop"
        if ($null -ne $script:OrigScreenSaverActive)  { Set-ItemProperty $desk -Name ScreenSaveActive  -Value $script:OrigScreenSaverActive  -Force -EA SilentlyContinue }
        if ($null -ne $script:OrigScreenSaverTimeout) { Set-ItemProperty $desk -Name ScreenSaveTimeOut -Value $script:OrigScreenSaverTimeout -Force -EA SilentlyContinue }
...
============================================================
  ...
  Machine:  $script:MachineMake $script:MachineModel
  Machine ID: $script:MachineID (unique to this PC -- a code derived from
...
function Write-Log {
    ...
    if (Test-Path (Split-Path $LogPath -Parent) -ErrorAction SilentlyContinue) {
        try { $entry | Out-File -FilePath $LogPath -Append -Encoding UTF8 -ErrorAction SilentlyContinue } catch { Write-Verbose "Non-fatal: $_" }
    }
}
...
function Show-Box {
    ...
    try { if ($Lines -and $Lines[0] -ne "---") { Write-Log -Message ("Rendered: " + $Lines[0].Trim()) -Status "SCREEN" } } catch { Write-Verbose "Non-fatal: $_" }
}
...
function Wait-ForUser {
    ...
    try {
        ...
        try { [Console]::TreatControlCAsInput = $true } catch { Write-Verbose "Non-fatal: $_" }
        do {
        ...
        try { Write-Log -Message ("Continue accepted at: " + (Get-PSCallStack)[1].Command) -Status "KEY" } catch { Write-Verbose "Non-fatal: $_" }
    } catch {
...
function Read-ValidKey {
    ...
    try {
        ...
        try { Write-Log -Message ("Key '" + $ch + "' accepted at: " + (Get-PSCallStack)[1].Command) -Status "KEY" } catch { Write-Verbose "Non-fatal: $_" }
        return $ch
    } catch {
...
function Read-NavKey {
    ...
    try {
        ...
        try { Write-Log -Message ("Nav '" + $result + "' at: " + (Get-PSCallStack)[1].Command) -Status "KEY" } catch { Write-Verbose "Non-fatal: $_" }
        return $result
    } catch {
        Write-Host ""
        do {
...
function Get-MachineIdentity {
    $script:MachineMake  = ""
    $script:MachineModel = ""
    $script:MachineID    = "UNKNOWN"
    try {
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop
        $script:MachineMake  = ("" + $cs.Manufacturer).Trim()
        $script:MachineModel = ("" + $cs.Model).Trim()
    } catch { Write-Verbose "Non-fatal: $_" }
    try {
        ...
        if ($uuid) {
            ...
            $script:MachineID = $hex.Substring(0, 12)
        }
    } catch { Write-Verbose "Non-fatal: $_" }
}
...
# ============================================================
function Show-FontInstruction {
    Write-Log -Message "[SCREEN-01] Rendered: Welcome / Font Instructions" -Status "SCREEN"
    Clear-Host
    Write-Host ""

    # ── ADMIN CHECK: If not running as admin, show clear instructions ──
    if (-not $script:IsAdmin) {
        Show-Box -Color White -Lines @(
        ...
        Wait-ForUser "  Press Enter or Space to CLOSE this window, then re-run as Administrator..."
    ...
    $introScreens = @(
        ...
        {   # Screen 3 of 5: Font setup (FT-04: now AFTER the welcome screens)
            ...
            Write-Host ("  This PC: " + $script:MachineMake + " " + $script:MachineModel + "  |  Machine ID: " + $script:MachineID) -ForegroundColor DarkCyan
            Write-Host ""
    ...
    while ($screenIdx -lt $introScreens.Count) {
        ...
        Write-Host ""
        if ($screenIdx -eq 0) {
            Wait-ForUser "  Press Enter or Space to continue..."
            $screenIdx++
        } else {
            $nav = Read-NavKey
...
function Test-PersonalComputer {
    ...
    Write-Log -Message "[SCREEN-05] Rendered: Personal Computer Check" -Status "SCREEN"
...
function Get-WinEdition {
    try {
        ...
        $script:WinEdition = if ($regEdition) { $regEdition } else { "Unknown" }
    } catch {
        try {
            $caption = (Get-CimInstance Win32_OperatingSystem -EA Stop).Caption
            $script:WinEdition = if ($caption -match "Pro") { "Professional" }
    ...
    $script:WinEditionFriendly = switch -Wildcard ($script:WinEdition) {
        "*Core*"           { "$script:WinEdition -- Windows 11 Home"; break }
        "*Home*"           { "$script:WinEdition -- Windows 11 Home"; break }
        "*Professional*"   { "$script:WinEdition -- Windows 11 Pro"; break }
        "*Pro*"            { "$script:WinEdition -- Windows 11 Pro"; break }
        "*Enterprise*"     { "$script:WinEdition -- Windows 11 Enterprise"; break }
        "*Education*"      { "$script:WinEdition -- Windows 11 Education"; break }
        default            { $script:WinEdition }
    }

    Write-Log -Message "Windows Edition: $script:WinEditionFriendly" -Status "INFO"

    $isHome = $script:WinEdition -notmatch "Pro|Enterprise|Education|Business"
    if ($isHome) {
        Clear-Host
        Write-Host ""
        Show-Box -Color White -Lines @(
            ...
            "---",
            "  Edition: $script:WinEditionFriendly                              ",
            "                                                                    ",
        ...
        Write-Host ""
        Write-Host "  OK  Windows Edition: $script:WinEditionFriendly" -ForegroundColor Green
        Write-Host ""
...
function Get-RAMStatus {
    try {
        ...
        $script:RAMGB = [math]::Round($ramBytes / 1GB)
        if ($script:RAMGB -le 8) {
            Clear-Host
            Write-Host ""
            Show-Box -Color White -Lines @(
                "  YOUR PC -- RAM: $($script:RAMGB) GB",
                "---",
            ...
            Write-Host ""
            Write-Log -Message "RAM: $($script:RAMGB) GB (minimal)" -Status "WARN"
            Wait-ForUser
        } elseif ($script:RAMGB -le 16) {
            Write-Host ""
            Write-Host "  Your PC has $($script:RAMGB) GB RAM -- most operations will run smoothly." -ForegroundColor Cyan
            ...
            Write-Log -Message "RAM: $($script:RAMGB) GB (moderate)" -Status "INFO"
        } else {
            Write-Log -Message "RAM: $($script:RAMGB) GB (ample)" -Status "INFO"
        }
    } catch {
        $script:RAMGB = 0
...
function Test-FirstRun {
    $script:IsFirstRun = -not (Test-Path $FirstRunFlag)
}
...
function Test-CheckpointReached {
    # Returns $true if $script:ResumeFrom checkpoint is at or before $Checkpoint
    ...
    $resumeIdx = $script:CheckpointOrder.IndexOf($script:ResumeFrom)
    $checkIdx  = $script:CheckpointOrder.IndexOf($Checkpoint)
...
function Show-ResumePrompt {
    ...
    # whether to resume or start fresh. Sets $script:ResumeFrom accordingly.
    ...
    if ($saved -and ($saved -in $script:CheckpointOrder)) {
        Clear-Host
        Write-Host ""
        Write-Log -Message "[SCREEN-25] Rendered: Welcome Back (Resume Prompt)" -Status "SCREEN"
        Show-Box -Color White -Lines @(
            "  WELCOME BACK                                              ",
            "---",
...
function Show-ResumeReverify {
    ...
    try {
        $rvBatt = Get-CimInstance -ClassName Win32_Battery -ErrorAction SilentlyContinue
        $script:HasBattery = ($null -ne $rvBatt)
        $script:OnBattery  = ($script:HasBattery -and $rvBatt.BatteryStatus -eq 1)
        $script:BatteryPct = if ($script:HasBattery -and $rvBatt.EstimatedChargeRemaining) { "$($rvBatt.EstimatedChargeRemaining)%" } else { "N/A" }
    } catch { $script:OnBattery = $false; $script:HasBattery = $false; $script:BatteryPct = "Unknown" }
    Enable-SleepPrevention
    if ($script:OnBattery) {
        Test-PowerStatus   # full battery warning screen -- worth showing
    } else {
        Write-Host "  OK  AC power confirmed (battery: $($script:BatteryPct)). Sleep prevention active." -ForegroundColor Green
        Write-Log -Message "Resume re-check: AC power confirmed ($($script:BatteryPct)). Sleep prevention active." -Status "OK"
    }
    Write-Host ""
    Wait-ForUser "  All re-checked. Press Enter or Space to continue where you left off..."
}
...
function Show-ScrollCopyTip {
    ...
    Write-Host ""
    Write-Log -Message "[SCREEN-02] Rendered: Scroll and Copy Tip" -Status "SCREEN"
    Wait-ForUser
    ...
    try {
        ...
        if ($confirm.ToUpper() -eq "N") {
            ...
            if ($openSettings.ToUpper() -eq "Y") {
                ...
                Wait-ForUser "  Press Enter or Space once you've corrected the time..."
...
function Show-SystemBaselineSummary {
    ...
    $lines += "  Windows:      $script:WinEdition"
    $lines += "  RAM:          $($script:RAMGB) GB"
    ...
    try {
        ...
        if ($batt) {
            ...
            if ($batt.EstimatedChargeRemaining -lt 50 -and -not $script:OnBattery) {
    ...
    Show-Box -Color White -Lines $lines
    Write-Host ""
...
function Show-PreScanGate {
    ...
    if ($script:ResumeFrom -eq "OfflineScanPending") {
    ...
    if ($script:IsFirstRun) {
        Write-Log -Message "[SCREEN-10] Rendered: Before We Scan Your PC" -Status "SCREEN"
        ...
        do {
            ...
            Write-Host "  N = Not ready yet -- exit this tool so you can prepare first" -ForegroundColor DarkGray
            Write-Host ""
            $ready = Read-ValidKey -ValidKeys @("Y","N","S") -Prompt "Your choice (Y = Scan now / S = Skip scan / N = Exit to prepare): "
            if ($ready.ToUpper() -eq "N") {
                Write-Host ""
                Write-Host "  N exits this tool so you can close programs and get ready." -ForegroundColor Yellow
                Write-Host "  Nothing is changed on your PC. Relaunch GatewayGuard when ready." -ForegroundColor Yellow
                Write-Host ""
...
function Show-PostScanGuidance {
    ...
    Write-Host ""
    Wait-ForUser "  Press Enter or Space to open Protection History..."
...
function Test-DefenderPrimary {
    ...
    try {
        ...
        if ($mbFree) {
            ...
            $defRT = $false
            try { $defRT = (Get-MpComputerStatus -EA Stop).RealTimeProtectionEnabled } catch { Write-Verbose "Non-fatal: $_" }
...
function Test-PowerStatus {
    ...
    try {
        ...
        $script:HasBattery = $hasBattery   # Store globally for power settings screen
        $script:OnBattery = ($hasBattery -and $battery.BatteryStatus -eq 1)
        $batteryPct = if ($hasBattery -and $battery.EstimatedChargeRemaining) { "$($battery.EstimatedChargeRemaining)%" } else { "N/A" }
        $script:BatteryPct = $batteryPct   # Store globally for power settings screen
    } catch {
        $script:OnBattery = $false
        $batteryPct = "Unknown"
        $hasBattery = $false
...
# ============================================================
function Invoke-PowerSettingsCheck {
    Clear-Host
    Write-Host ""
    ...
    # Use $script:HasBattery set during step 9 power check -- avoids re-querying WMI
    try {
        $hasBatteryPS = if ($null -ne $script:HasBattery) { $script:HasBattery } else {
            $null -ne (Get-CimInstance -ClassName Win32_Battery -EA SilentlyContinue)
        }
        if (-not $hasBatteryPS) {
    ...
    Wait-ForUser "  Press Enter or Space to see the power settings review..."
    ...
    Set-PowerSetting -Choices $powerChoices
    Wait-ForUser "  Power settings complete. Press Enter or Space to continue..."
}
...
function Invoke-AppsAudit {
    ...
    foreach ($path in $regPaths) {
        try {
            ...
            ForEach-Object {
                $lastUsed = $null
                if ($_.InstallDate) {
                    try { $lastUsed = [datetime]::ParseExact($_.InstallDate, "yyyyMMdd", $null) } catch { Write-Verbose "Non-fatal: $_" }
                }
    ...
    Wait-ForUser "  Apps Audit complete. Press Enter or Space to continue..."
}
...
function Get-MalwarebytesState {
    ...
    try {
        ...
        try {
            $fwProducts = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName FirewallProduct -EA SilentlyContinue
...
function Get-AllStatus {
    ...
    foreach ($s in $Settings) {
        $ggChkNum++
        try { Write-Host ("`r  Checking setting $ggChkNum of $($Settings.Count)...   ") -ForegroundColor Cyan -NoNewline } catch { Write-Verbose "Non-fatal: $_" }
        if ($s.SkipOnHome -and $isHome) { $s.Status = "N/A on Home Edition -- GOOD"; continue }
        if ($s.RequiresAdmin -and -not $script:IsAdmin) { $s.Status = "Requires Admin Access"; continue }
        ...
        switch ($s.ID) {
            ...
            2 {
                try {
                    ...
                    $avP2  = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA SilentlyContinue
            ...
            5 {
                try {
                    ...
                    $avP5    = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA Stop
    ...
    try { Write-Host ("`r  All $($Settings.Count) settings checked.                    ") -ForegroundColor Green } catch { Write-Verbose "Non-fatal: $_" }
...
# ============================================================
function Test-NonRecommendedSelection {
    ...
    do {
        ...
        if ($resp.ToUpper() -eq "S") {
            ...
            try { while ($Host.UI.RawUI.KeyAvailable) { $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown") } } catch { Write-Verbose "Non-fatal: $_" }
            Clear-Host
            Write-Host ""
            Show-Box -Color White -Lines @(
            ...
            Wait-ForUser "  Press Enter or Space to return to the HEADS UP screen..."
        }
...
function Set-SecuritySetting {
    ...
    if ($Setting.RequiresAdmin -and -not $script:IsAdmin) {
    ...
    switch ($Setting.ID) {
        ...
        2 {
            try {
                ...
                $avPA2   = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA SilentlyContinue
        ...
        3 {
            ...
            $avPA3  = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA SilentlyContinue
        ...
        5 {
            try {
                ...
                $avPA5  = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA SilentlyContinue
        ...
        16 {
            try {
                $avP16  = Get-CimInstance -Namespace "root\SecurityCenter2" -ClassName AntiVirusProduct -EA SilentlyContinue
...
function Show-ScopeDisclaimer {
    ...
    if (-not $script:SleepPrevented) { Enable-SleepPrevention }
    ...
    $script:HasPasswordManager = ($pmAns.ToUpper() -eq "Y")
    if (-not $script:HasPasswordManager) {
    ...
    Show-Box -Color White -Lines @(
        ...
        $(if ($script:HasPasswordManager) { "  [E] Edge Password Save  -- Keeps passwords out of the browser    " } else { "  [E] Edge Password Save  -- SKIPPED: set up a password manager 1st" }),
        "                                                                    ",
        "  WHAT IT DOES NOT DO:                                             ",
        ...
        "                                                                    ",
        "  Edition: $script:WinEditionFriendly",
        "  Admin:   $(if ($script:IsAdmin) { 'Full access -- all settings available' } else { 'Limited -- some settings skipped' })",
        "  Power:   $(if ($script:OnBattery) { 'BATTERY -- plug in before BitLocker' } else { 'AC power OK' })  Sleep: $(if ($script:SleepPrevented) { 'ACTIVE' } else { 'inactive' })"
    )
    Write-Host ""
    ...
    if ($ggScopeNav -eq "BACK") {
        ...
        $script:HasPasswordManager = ($pmAns2.ToUpper() -eq "Y")
        $eSetting2 = $Settings | Where-Object { $_.ID -eq 15 }
        if ($eSetting2) { $eSetting2.Selected = $script:HasPasswordManager }
        Write-Log -Message "Password-manager answer revised via Back: $($script:HasPasswordManager)" -Status "INFO"
    }
...
function Show-ConvenienceReview {
    ...
    Write-Host ""
    Wait-ForUser "  Press Enter or Space to review each item..."

    $ggCITotal = $convItems.Count   # FT-74 (ascii31)
    $ggCIIdx   = 0
    foreach ($ci in $convItems) {
        ...
        Write-Host ""
        Show-Box -Color White -Lines @(
            "  CONVENIENCE FEATURE [$ggCIIdx of $ggCITotal]: $($ci.Name)          ",   # FT-74
            "---",
            "  WHAT WAS DONE:                                              ",
            "  $($ci.What)                                                 ",
            "---",
        ...
        if ($resp.ToUpper() -eq "N") {
            ...
            # FT-79 (ascii31): removed 1500ms post-revert sleep
        } else {
            ...
            # FT-79 (ascii31): removed 800ms post-keep sleep
        }
    }
    ...
    Write-Host ""
    Wait-ForUser "  Press Enter or Space to see your manual steps checklist..."
}
...
function Initialize-ScheduledTask {
    ...
    try {
        ...
        # FT-76 (ascii31): use 4 Once triggers (PS 5.1 compat)
        $ggQtrYear = (Get-Date).Year
        $trigger1 = @(
            (New-ScheduledTaskTrigger -Once -At "$ggQtrYear-01-01 02:00"),
            (New-ScheduledTaskTrigger -Once -At "$ggQtrYear-04-01 02:00"),
            (New-ScheduledTaskTrigger -Once -At "$ggQtrYear-07-01 02:00"),
            (New-ScheduledTaskTrigger -Once -At "$ggQtrYear-10-01 02:00")
        )
...
"@
        ...
        # FT-76 (ascii31): 12 Once triggers, 1st of each month @ 10AM
        $ggMoYear = (Get-Date).Year
        $trigger2 = 1..12 | ForEach-Object { New-ScheduledTaskTrigger -Once -At (("{0}-{1:D2}-01 10:00") -f $ggMoYear, $_) }
...
function Get-BitLockerTimeEstimate {
    try {
        ...
        $ramGB = if ($script:RAMGB -gt 0) { $script:RAMGB } else { 8 }
        ...
        return [PSCustomObject]@{ DriveGB=0; DriveType="Unknown"; RAMGB=$script:RAMGB; Estimate="several hours"; Severity="high" }
    }
}

function Save-BitLockerKey {
    param($Key)
    $script:BitLockerKeyPath = "$([Environment]::GetFolderPath('Desktop'))\BitLocker-Recovery-Key-$(Get-Date -Format 'yyyy-MM-dd').txt"   # FT-75
    @"
========================================================
  BITLOCKER / DEVICE ENCRYPTION RECOVERY KEY
  Generated: $(Get-Date)
  Computer:  $env:COMPUTERNAME
  Windows:   $script:WinEdition
========================================================
...
========================================================
"@ | Out-File -FilePath $script:BitLockerKeyPath -Encoding UTF8
}
...
function Show-BitLockerWhyEncrypt {
    ...
    Wait-ForUser "  Press Enter or Space to return..."
}
...
function Show-BitLockerScreen {
    ...
    try {
        $blBatt = Get-CimInstance -ClassName Win32_Battery -EA SilentlyContinue
        ...
        $script:OnBattery = $blOnBattery
    } catch {
        $blOnBattery = $script:OnBattery
        $blBattPct   = "Unknown"
    }
    ...
    if ($blOnBattery) {
        ...
        $blBatt2 = Get-CimInstance -ClassName Win32_Battery -EA SilentlyContinue
    ...
    Write-Host ""
    Write-Host "  The next screen lets this tool save your key and start" -ForegroundColor Cyan
    ...
    if ($blPrepConfirm.ToUpper() -eq "N") {
        Write-Host ""
        Write-Host "  No problem. Run this tool again when you are ready." -ForegroundColor Yellow
    ...
    try {
        ...
        $script:BitLockerKeyGenerated = $true
        ...
        Show-Box -Color White -Lines @(
            ...
            "  $(Split-Path $script:BitLockerKeyPath -Leaf)",
            "                                                           ",
        ...
        Write-Host ""
        Wait-ForUser "  Press Enter or Space ONLY after you have SAVED and PRINTED the recovery key..."
        $script:BitLockerKeyGenerated = $false
    } catch {
        Write-Host ""
        Write-Host "  ERROR: $_" -ForegroundColor Red
...
function Show-ModeSelector {
    ...
    Show-Box -Color White -Lines @(
        ...
        "  [3] EXIT                                                  ",
        "                                                            ",
        "  Admin status: $(if ($script:IsAdmin) { 'FULL ACCESS OK' } else { 'LIMITED MODE -- some settings unavailable' })",
        "  Edition: $script:WinEditionFriendly"
    )
    Write-Host ""
}
...
function Start-ConsoleMode {
    trap {
        ...
        Wait-ForUser "  Press Enter or Space to exit..."
        Disable-SleepPrevention
        Save-Log
        exit
    }
    ...
    :checklistLoop while ($true) {
        ...
        try { if ($Host.UI.RawUI.WindowSize.Width -ge 116) { $ggNameW = 63; $ggStatW = 38 } } catch { Write-Verbose "Non-fatal: $_" }
        ...
        foreach ($s in $ggPageItems) {
            ...
            $auto     = if (-not $s.CanAuto) { "*" } elseif ($s.RequiresAdmin -and -not $script:IsAdmin) { "!" } else { "" }
        ...
        # same safe fallback pattern already used in Read-ValidKey/Wait-ForUser.
        ...
        switch ($userInput.ToUpper()) {
            ...
            "R" {
                ...
                $proceed = Test-NonRecommendedSelection -Stage "review"
                ...
                if ($blItem -and -not $blItem.Selected -and $blItem.Status -match "NOT Encrypted") {
                    ...
                    if ($blDecision -eq "GoBack") {
                        ...
                        Wait-ForUser "  Press Enter or Space to return to the checklist..."
                        continue checklistLoop
                    }
                }
                ...
                $proceed2 = Test-NonRecommendedSelection -Stage "final"
                ...
                foreach ($s in ($Settings | Where-Object { $_.Selected -and $_.ID -ne 8 })) {
                        ...
                        switch ($goodResp.ToUpper()) {
                            ...
                            "Y" {
                                ...
                                if ($warnResp.ToUpper() -eq "Y") {
                                    ...
                                    $result = Set-SecuritySetting -Setting $s
                    ...
                    if (-not $s.CanAuto) {
                        ...
                        $r = Set-SecuritySetting -Setting $s
                        Write-Host "  INSTRUCTIONS: $r" -ForegroundColor Yellow
                        Wait-ForUser
                        continue
                    }
                    if ($s.RequiresAdmin -and -not $script:IsAdmin) {
                    ...
                    if ($s.SkipOnHome -and ($script:WinEdition -notmatch "Pro|Enterprise|Education")) {
                    ...
                    if ($changeGood.ToUpper() -eq "Y") {
                        foreach ($item in $goodItems) {
                            ...
                            if ($reapply.ToUpper() -eq "Y") {
                                ...
                                $result = Set-SecuritySetting -Setting $item
                                Write-Host "  Result: $result" -ForegroundColor Cyan
                            }
                        }
                    }
                ...
                if ($blSetting -and $blSetting.Selected) {
                    # G-02b (ascii32): Mark mode reminder before BitLocker screen
                    # -- this screen shows the Recovery Key, which users must copy
                    Clear-Host
                    Write-Host ""
                    ...
                    Write-Host ""
                    Wait-ForUser "  Press Enter or Space to continue to the next step..."
...
function Start-GUIMode {
    ...
    $form.Text    = "GatewayGuard -- Windows 11 Security Hardening Tool v$ScriptVersion -- $script:WinEditionFriendly"
    ...
    $form.Add_FormClosing({
        # Required positional signature for the .NET FormClosingEventHandler
        # delegate (object sender, FormClosingEventArgs e) -- $formSender is
        # unused by design; renamed from $sender to avoid PSScriptAnalyzer's
        # automatic-variable-collision warning.
        param($formSender, $e)
        [void]$formSender
    ...
    $adminColor = if ($script:IsAdmin) { [System.Drawing.Color]::FromArgb(0,200,100) } else { [System.Drawing.Color]::Orange }
    $lblStatus = New-Object System.Windows.Forms.Label
    $lblStatus.Text = "Edition: $script:WinEdition  |  Admin: $(if ($script:IsAdmin) { 'Full Access OK' } else { 'Limited Mode -- some settings unavailable' })"
    ...
    foreach ($s in $Settings) {
        $isSkipped = ($s.SkipOnHome -and ($script:WinEdition -notmatch "Pro|Enterprise|Education")) -or ($s.RequiresAdmin -and -not $script:IsAdmin)
...
# Then: mode selection -> Start-ConsoleMode or Start-GUIMode
...
Write-Host "  This PC: $script:MachineMake $script:MachineModel  |  Machine ID: $script:MachineID" -ForegroundColor DarkGray
...
Write-Log -Message "Machine: $script:MachineMake $script:MachineModel | MachineID: $script:MachineID" -Status "INFO"

# Detect admin status IMMEDIATELY -- must happen before Show-FontInstruction,
# which checks $script:IsAdmin. (BUGFIX ascii23: this used to run too late,
...
if (-not $script:ResumeFrom) {
    Show-FontInstruction
...
# FT-66 (ascii29): global error trap -- see comment at try above
    ...
    try { Save-Log } catch { Write-Verbose "Non-fatal: $_" }
    try { Disable-SleepPrevention } catch { Write-Verbose "Non-fatal: $_" }
    try { Restore-ScreenSaver } catch { Write-Verbose "Non-fatal: $_" }
    Write-Host ""
...
#     'Get-WinEdition','Test-DefenderPrimary','Get-AllStatus','Set-SecuritySetting',
#     'Start-ConsoleMode','Start-GUIMode','Show-BitLockerScreen','Write-Log',
#     'Save-Log','Show-Box','Test-PersonalComputer','Test-AdminAccess'
# )
