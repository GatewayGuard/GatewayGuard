# Dated: 2026-07-12 03:20 ET
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii27.ps1
# BUILD:   ascii27  |  Version 3.1
...
function Run-PowerSettingsCheck {
    ...
    try {
        ...
        if (-not $hasBatteryPS) {
            ...
            # FT-36 (2026-07-12): both powercfg reads failed on the Dell field
            # unit ("Could not read"). Capture raw output so the log shows WHY.
            if ($null -eq $cbVal) {
                $cbRaw = ($cb | Select-Object -First 6) -join " | "
                Write-Log -Message "FT-36 powercfg BATACTIONCRIT parse failed. Raw: $cbRaw" -Status "WARN"
            }
...
function Run-ConsoleMode {
    ...
    :checklistLoop while ($true) {
        ...
        foreach ($s in $Settings) {
            ...
            $nameStr  = if ($nameStr.Length -gt 42) { $nameStr.Substring(0,40) + ".." } else { $nameStr.PadRight(42) }
            $statusStr = if ($s.Status.Length -gt 28) { $s.Status.Substring(0,26) + ".." } else { $s.Status.PadRight(28) }
