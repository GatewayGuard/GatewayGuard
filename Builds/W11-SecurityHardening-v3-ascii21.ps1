# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii21.ps1
# BUILD:   ascii21  |  Version 3.0
...
function Pause-ForUser {
    ...
    if ($Message) { Write-Host "" ; Write-Host $Message -ForegroundColor DarkGray }
    do {
        $k = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    } while ($k.VirtualKeyCode -notin @(13, 32))  # 13=Enter 32=Space only
}
...
function Show-FontInstructions {
    Clear-Host
    Write-Host ""
    # Window size instruction -- shown FIRST before anything else
    Draw-Box -Color White -Lines @(
        "  BEFORE YOU START -- WINDOW SETUP                         ",
        "---",
        "  For the best experience:                                 ",
        "  1. MAXIMIZE this window now (press Windows key + Up      ",
        "     arrow, or drag window to fill your screen)            ",
        "  2. SCROLLING: Some screens are longer than your window.  ",
        "     Scroll arrows appear at the TOP RIGHT and BOTTOM RIGHT ",
        "     corners of the window. If the bottom arrow disappears, ",
        "     move your mouse to the bottom right corner and it will  ",
        "     reappear. Always scroll to the BOTTOM before pressing  ",
        "     Enter or Space to continue.                            ",
        "  3. PAGE UP / PAGE DOWN keys scroll quickly if available. ",
        "  4. You can also use the scroll wheel on your mouse.      "
    )
    Write-Host ""
...
function Show-PreScanGate {
    ...
    if ($global:IsFirstRun) {
        ...
        Write-Host ""
        Write-Host "  Complete BOTH scans above, then return here and press Enter to continue." -ForegroundColor Yellow
        Write-Host ""

        do {
            $done = Read-ValidKey -ValidKeys @("Y","N") -Prompt "Have you completed BOTH scans? (Y = Yes / N = Exit to run scans): "
...
function Test-PowerStatus {
    ...
    if ($global:OnBattery) {
        ...
        if ($hasBattery) {
            ...
            Write-Host "      Detected via: Win32_Battery (BatteryStatus = $($battery.BatteryStatus), 100% = fully charged/plugged in)" -ForegroundColor DarkGray
            Write-Host "      Battery level: $batteryPct  |  Safe to run all settings including BitLocker." -ForegroundColor DarkGray
        } else {
            Write-Host "  OK  AC power -- desktop PC detected (no Win32_Battery object found)." -ForegroundColor Green
        }
        Write-Host "  OK  Sleep prevention active -- PC will not sleep during this run." -ForegroundColor Green
        Write-Host "      NOTE: This was SET BY THIS TOOL -- your original sleep setting will be restored on exit." -ForegroundColor DarkGray
...
function Show-ScopeDisclaimer {
    ...
    Write-Host ""
    Pause-ForUser "  Read the above, then press any key to see the security checklist..."
}
...
function Show-ConvenienceReview {
    ...
    Write-Host ""
    Pause-ForUser "  Press any key to begin reviewing each item..."
    ...
    Write-Host ""
    Pause-ForUser "  Press any key to see your manual steps checklist..."
}
