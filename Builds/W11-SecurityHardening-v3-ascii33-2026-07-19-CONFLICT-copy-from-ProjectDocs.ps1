# Dated: 2026-07-19 18:15 ET
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii33-2026-07-19.ps1
...
function Show-StepHeader {
    ...
    Write-Host "  $Section -- Step $ggN" -ForegroundColor DarkCyan
    Write-Host ""
}
...
function Get-AllStatuses {
    ...
    foreach ($s in $Settings) {
        ...
        switch ($s.ID) {
            ...
            12 {
                try { $dd = (Get-ItemProperty "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" -EA SilentlyContinue).AllowTelemetry; $s.Status = if ($null -ne $dd -and $dd -le 1) { "Required Only -- GOOD" } else { "Sending extra data -- we will limit it" } }
...
function Setup-ScheduledTasks {
    ...
    try {
        ...
        $ggT1Out = & schtasks.exe /create /f /tn "$ggT1Name" `
            /tr "`"$mpCmd`" -Scan -ScanType 4" `
            /sc monthly /m JAN,APR,JUL,OCT /d 1 /st 02:00 `
            /ru "SYSTEM" /rl HIGHEST 2>&1
        if ($LASTEXITCODE -ne 0) { throw "schtasks exit $LASTEXITCODE -- $ggT1Out" }
...
"@
        ...
        $ggT2Out = & schtasks.exe /create /f /tn "$ggT2Name" `
            /tr "powershell.exe -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptDir\MBReminder.ps1`"" `
            /sc monthly /d 1 /st 10:00 2>&1
        if ($LASTEXITCODE -ne 0) { throw "schtasks exit $LASTEXITCODE -- $ggT2Out" }
