# Dated: 2026-07-28 13:30 EDT
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii37-2026-07-28-1330.ps1
# BUILD:   ascii37  |  Version 3.1
...
function Draw-Box {
    ...
    $border = "+" + ("=" * $Width) + "+"
    # FT-122 (ascii37): the screen number is stamped INTO the top border, not
    # into a content line. PREVENTS: any change to box width, and therefore
    # any regression of FT-117 (content cut off the right edge on a narrow
    # console) -- the tag overwrites border characters that were already
    # there, so a numbered box and an unnumbered box are exactly the same
    # width. COULD CAUSE: nothing layout-related. The only new failure mode
    # would be a tag wider than the box, which the guard below makes
    # impossible: the widest possible tag is 13 characters and the minimum
    # box width is 44.
    $topBorder = $border
    if ($ScreenId) {
        $ggTag = "[ Screen " + $ScreenId + " ]"
    ...
    try {
        if ($Lines -and $Lines[0] -ne "---") {
            ...
            if ($ScreenId) {
                Write-Log -Message ("[SCREEN-" + $ScreenId + "] Rendered: " + $ggTitle) -Status "SCREEN"
            } else {
...
function Show-ScopeDisclaimer {
    ...
    Write-Host ""
    Pause-ForUser "  Press Enter or Space to continue to what this tool does..."
    }
    ...
    while ($true) {
        ...
        if ($ggScopePage -eq 1) {
            Draw-Box -ScreenId "54" -Color White -Lines @(
                "  WHAT THIS TOOL DOES AND DOES NOT DO   (page 1 of 2)              ",
                "---",
                "  HOW THIS WORKS:                                                   ",
            ...
            Draw-Box -ScreenId "75" -Color White -Lines @(
                "  WHAT THIS TOOL DOES AND DOES NOT DO   (page 2 of 2)              ",
                "---",
...
function Show-BitLockerHomeScreen {
    ...
    Write-Host ""
    Write-Host "  Checking your PC's requirements for Device Encryption..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Log -Message "BitLocker/Device Encryption: Home edition -- manual path shown, no changes made by this tool" -Status "SKIP"
    Pause-ForUser
}
    ...
    if ($choice -eq "2") {
        if ($blSleepChanged) {
            ...
            if ($blSleepAck.ToUpper() -eq "N") {
                Write-Host ""
                Write-Host "  Set Sleep and Display to Never in Settings, then run this tool again to enable BitLocker." -ForegroundColor Yellow
...
function Run-ConsoleMode {
    ...
    :checklistLoop while ($true) {
        ...
        try {
            ...
            Write-Log -Message ("Checklist render: page " + $script:ChecklistPage + ", window width " + $ggLogW + ", " + $ggSelN + " item(s) selected") -Status "SCREEN"
        } catch {}
        Write-Host ""
        ...
        # FT-125 (ascii37): [int] casts. [Math]::Floor returns a DOUBLE, so
        # $ggNameW and $ggStatW were doubles being handed to Substring(Int32,
        # Int32) and PadRight(Int32) at the two render lines below. PowerShell
        # coerces this in the normal case, but it is exactly the loose typing
        # at a computed boundary that Class 4 rule 2 exists to prevent, on the
        # exact lines field note 14's crash points at. Cheap, and it removes
        # the question instead of leaving it to be re-argued next round.
        $ggNameW = [int][Math]::Floor($ggAvailable * 0.6)
        $ggStatW = [int]($ggAvailable - $ggNameW)
        if ($ggNameW -lt 20) { $ggNameW = 20 }
        ...
        switch ($userInput.ToUpper()) {
            ...
            "R" {
                ...
                Pause-ForUser "  Press Enter or Space to close GatewayGuard..."
                return
            }
            default {
...
function Run-GUIMode {
    ...
    $btnRun.Add_Click({
        ...
        [System.Windows.Forms.MessageBox]::Show(
            "All selected settings processed. GatewayGuard will now close.`n`nLog: $(Split-Path $LogPath -Leaf)`n`nSee the console window for your manual steps checklist -- complete those on your own time.`n`nGuide: $GuideURL",
            "Run Complete -- GatewayGuard Closing", "OK", "Information"
        )
        $form.Close()
    })
