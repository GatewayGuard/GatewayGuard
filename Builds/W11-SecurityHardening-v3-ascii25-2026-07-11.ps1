...
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii25.ps1
# BUILD:   ascii25  |  Version 3.1
...
function Pause-ForUser {
    ...
    try {
        # FT-01 (2026-07-11): flush any stale/buffered keys first -- switching
        # away to another window and back can leave key events queued that
        # would instantly (and invisibly) satisfy this prompt.
...
function Read-ValidKey {
    ...
    try {
        # FT-01 (2026-07-11): discard stale buffered keys from focus switches
        $Host.UI.RawUI.FlushInputBuffer()
        do {
            $k = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
...
function Test-DefenderPrimary {
    ...
    try {
        ...
        if ($defenderRTOn) {
            ...
            # Defender real-time is CONFIRMED off and no known 3rd-party AV
            Clear-Host
            Write-Host ""
            Draw-Box -Color White -Lines @(
...
function Show-BitLockerScreen {
    ...
    if ($blOnBattery) {
        ...
        try {
            $Host.UI.RawUI.FlushInputBuffer()   # FT-01: discard stale keys
