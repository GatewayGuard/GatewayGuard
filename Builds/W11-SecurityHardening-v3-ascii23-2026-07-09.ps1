...
function Enable-SleepPrevention {
    try {
        # BUGFIX ascii23 (2026-07-09): The previous fix re-cast the COMBINED
        # -bor result to [uint32], but this still failed in real testing --
        # PowerShell's -bor operator is apparently producing a signed result
        # here regardless of operand/result casting. Eliminating the runtime
        # bitwise operation entirely: ES_CONTINUOUS | ES_SYSTEM_REQUIRED |
        # ES_DISPLAY_REQUIRED (0x80000000 | 0x1 | 0x2) is a FIXED constant
        # that never changes, so it's hardcoded directly instead of computed
        # at runtime. This sidesteps whatever is going wrong with -bor.
        $flags = [uint32]0x80000003   # = ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED
...
function Show-FontInstructions {
    ...
    Write-Host ""
    Pause-ForUser "  Got it -- press Enter to continue..."
    Clear-Host
    Write-Host ""
...
function Get-WinEdition {
    ...
    $global:WinEditionFriendly = switch -Wildcard ($global:WinEdition) {
        "*Core*"           { "$global:WinEdition -- Windows 11 Home" }
        "*Home*"           { "$global:WinEdition -- Windows 11 Home" }
        "*Professional*"   { "$global:WinEdition -- Windows 11 Pro" }
        "*Pro*"            { "$global:WinEdition -- Windows 11 Pro" }
        "*Enterprise*"     { "$global:WinEdition -- Windows 11 Enterprise" }
        "*Education*"      { "$global:WinEdition -- Windows 11 Education" }
...
function Test-TimeDateSync {
    ...
    try {
        ...
        if ($confirm.ToUpper() -eq "N") {
            Write-Host "  Please correct this manually in Settings > Time & Language before continuing." -ForegroundColor Yellow
    ...
    switch ($Setting.ID) {
        ...
        6 {
            ...
            try {
                ...
                Write-Host ""
                Pause-ForUser "  Press Enter to continue..."
            } catch {
...
function Run-ConsoleMode {
    ...
    :checklistLoop while ($true) {
        ...
        Write-Host "  |  | GatewayGuard Security Hardening v3.0              | Console Mode                       |" -ForegroundColor White
        Write-Host "  +--+----------------------------------------------------+------------------------------------+" -ForegroundColor White
        Write-Host "  |  | Setting                                            | Status                             |" -ForegroundColor White
        Write-Host "  +--+----------------------------------------------------+------------------------------------+" -ForegroundColor White
        ...
        foreach ($s in $Settings) {
            ...
            $nameStr  = if ($nameStr.Length -gt 52) { $nameStr.Substring(0,52) } else { $nameStr.PadRight(52) }
            $statusStr = if ($s.Status.Length -gt 36) { $s.Status.Substring(0,36) } else { $s.Status.PadRight(36) }
...
if ($firstCh -in @("R","A","N","Q")) {
    ...
    } elseif ($secondKey.VirtualKeyCode -in @(13,32)) {
...
# 1. Opening sequence -- maximize + scroll-to-top (UX-01, UX-02, UX-03)
...
if (-not $global:ResumeFrom) {
    ...
    Write-Host ""
    Write-Host "  Now scroll to the top of this window using the scrollbar" -ForegroundColor White
    Write-Host "  on the right side, so you don't miss anything." -ForegroundColor White
    Write-Host ""
    Pause-ForUser "  Press Enter or Space once you've scrolled to the top..."
}

# 2. Font instructions (skip if resuming -- already seen)
if (-not $global:ResumeFrom) {
