# Dated: 2026-07-10 02:10 EDT
# ================================================================
# FILE:    W11-SecurityHardening-v3-ascii23.ps1
# BUILD:   ascii23  |  Version 3.1
...
# CHANGES FROM ascii22 (see GatewayGuard_ascii23_DevNotes.md for full detail):
#   UX-01/02/03: Opening sequence -- maximize window, scroll-to-top, readable pacing
#   UX-04:       Warning before offline scan reboot (was silent/no warning)
#   UX-05/06:    State/resume system -- survives the offline-scan reboot,
#                jumps straight back to the right checkpoint
#   UX-07:       Guided Protection History walkthrough after reboot
#   UX-08:       Time & date sync check (new section)
#   UX-09:       Scan duration expectations set before scanning
#   UX-10:       System Baseline Summary as Step 0
#   UX-11:       Pre-scan and pre-BitLocker prep checklist
#   OBS-01:      UAC/permission dialog timeout warning + recovery steps
#   NEW:         Defender Offline Scan is now ACTUALLY AUTOMATED via
#                Start-MpWDOScan (ascii22 only gave manual instructions)
#   NEW:         Malwarebytes detect-and-launch flow (was manual-only)
# ================================================================
...
function Show-FontInstructions {
    ...
    if (-not $global:IsAdmin) {
        Draw-Box -Color White -Lines @(
            ...
            "  3. RIGHT-CLICK on Run-GatewayGuard.bat                      ",
        ...
        Write-Host ""
        Pause-ForUser "  Press Enter or Space to continue in limited mode, or close and re-run as Admin..."
    }

    # ── WINDOW SETUP shown FIRST before anything else ──
    Draw-Box -Color White -Lines @(
        "  BEFORE YOU START -- WINDOW SETUP                              ",
        "---",
        "  For the best experience please do the following:              ",
        "                                                                 ",
        "  1. MAXIMIZE THIS WINDOW NOW:                                  ",
        ...
        "                                                                 ",
        "  3. ALWAYS scroll to the BOTTOM of every screen before        ",
        "     pressing Enter or Space to continue -- there may be        ",
        "     important information below what you can first see.        ",
        "                                                                 ",
    ...
    Write-Host ""
    Pause-ForUser "  Got it -- press Enter or Space to continue..."
    Clear-Host
    Write-Host ""
    ...
    Write-Host ""
    Pause-ForUser "  Font looks good? Press Enter or Space to continue..."
    ...
    Write-Host ""
    Pause-ForUser "  Ready to begin? Press Enter or Space to begin..."
}
...
function Test-TimeDateSync {
    ...
    try {
        ...
        if ($confirm.ToUpper() -eq "N") {
            ...
            Write-Host ""
            Write-Host "  GatewayGuard can open Windows' Date & Time settings now" -ForegroundColor Yellow
            Write-Host "  so you can correct it." -ForegroundColor Yellow
            $openSettings = Read-ValidKey -ValidKeys @("Y","N") -Prompt "  Open Date & Time settings now? (Y/N): "
...
function Show-PreScanGate {
    ...
    if ($global:IsFirstRun) {
        ...
        Write-Host ""
        do {
            $ready = Read-ValidKey -ValidKeys @("Y","N") -Prompt "Ready to continue? (Y = Yes / N = Exit to prepare): "
...
function Show-MalwarebytesFollowUp {
    ...
    if ($mbState -eq "NotInstalled") {
        ...
        Draw-Box -Color White -Lines @(
            ...
            "---",
            "  GatewayGuard can open Malwarebytes now so you can run a    ",
            "  quick scan -- a good second opinion alongside Defender.    "
        )
        Write-Host ""
        ...
        if ($runMb.ToUpper() -eq "Y") {
            ...
            if ($mbExe) {
                Start-Process $mbExe
                Write-Host "  Malwarebytes should now be open. Click 'Scan' to check for threats." -ForegroundColor Green
                Write-Log -Message "Launched Malwarebytes at $mbExe" -Status "INFO"
                Write-Host ""
                ...
                if ($mbState -eq "TrialActive") {
                    ...
                    Draw-Box -Color White -Lines @(
                        ...
                        "                                                            ",
                        "  While your trial is active, you have access to a DEEPER   ",
                        "  scan option. To use it: click Scan options, then choose    ",
                        "  Deep Scan (may take longer, but checks more thoroughly).   ",
                        "  This option may not be available once the trial ends.     "
                    )
                ...
                Write-Host "  location. Please open it manually from the Start Menu." -ForegroundColor Yellow
                Write-Log -Message "Malwarebytes detected but executable not found at expected paths" -Status "WARN"
            }
        } else {
...
function Get-MalwarebytesState {
    ...
    try {
        ...
        return if ($mbPremium) { "TrialActive" } else { "FreeCompanion" }
                ...
                switch ($tpState) {
                    ...
                    "Off" {
                        $s.Status = if ($mbSt3 -eq "TrialActive") {
                            "OFF -- Malwarebytes Premium Trial is active as VP. Tamper Protection is off. Enable in Windows Security -> V&T protection settings."
                        } else {
...
function Test-NonRecommendedSelections {
    ...
    do {
        $resp = Read-ValidKey -ValidKeys @("Y","N","S") -Prompt "Your choice (Y = Keep / N = Revert / S = Skip): "
        switch ($resp.ToUpper()) {
            "S" {
                Write-Host ""
                foreach ($item in $criticalDeselected) {
...
function Show-ScopeDisclaimer {
    ...
    Draw-Box -Color White -Lines @(
        ...
        "                                                                    ",
        "  WHAT IT CHANGES (with your approval):                            ",
        "  * Security features ON  (Defender, Tamper Protection, BitLocker) ",
        "  * Risky features OFF    (Wake-on-LAN, Fast Startup)              ",
        ...
        "  [E] Edge Password Save  -- Keeps passwords out of the browser    ",
        "                                                                    ",
        "  WHAT IT DOES NOT DO:                                             ",
    ...
    Write-Host ""
    Pause-ForUser "  Read the above, then press Enter or Space to see the security checklist..."
}
...
function Run-ConsoleMode {
    ...
    :checklistLoop while ($true) {
        Clear-Host
        Write-Host ""
        # BUGFIX ascii23 (2026-07-09): original table was 97 characters wide
        # (exceeds the 80-column standard PowerShell console width, causing
        # the right-side text to be cut off) AND the header/border had an
        # extra unused column segment that didn't match the actual 2-column
        # data rows below. Resized to 79 chars total, and header/border now
        # match the real row structure exactly.
        Write-Host "  +--------------------------------------------+------------------------------+" -ForegroundColor White
        Write-Host ("  | " + "GatewayGuard Security Hardening v$ScriptVersion".PadRight(42) + " | " + "Console Mode".PadRight(28) + " |") -ForegroundColor White
        Write-Host "  +--------------------------------------------+------------------------------+" -ForegroundColor White
        Write-Host ("  | " + "Setting".PadRight(42) + " | " + "Status".PadRight(28) + " |") -ForegroundColor White
        Write-Host "  +--------------------------------------------+------------------------------+" -ForegroundColor White
        ...
        foreach ($s in $Settings) {
            ...
            $nameStr  = if ($nameStr.Length -gt 42) { $nameStr.Substring(0,42) } else { $nameStr.PadRight(42) }
            $statusStr = if ($s.Status.Length -gt 28) { $s.Status.Substring(0,28) } else { $s.Status.PadRight(28) }
        ...
        Write-Host "  Enter command (R/A/N/Q or item number 1-19): " -ForegroundColor White -NoNewline
        $userInput = ""
        try {
            ...
            "R" {
                ...
                if ($applyItems) {
                    foreach ($s in $applyItems) {
                        $statusShort = if ($s.Status.Length -gt 28) { $s.Status.Substring(0,28) } else { $s.Status }
...
# 1. Font instructions FIRST (FT-04: was previously after Welcome/maximize --
# moved to be the literal first screen, per field-test feedback)
if (-not $global:ResumeFrom) {
    Show-FontInstructions
}

# 2. Opening sequence -- maximize + scroll-to-top (UX-01, UX-02, UX-03)
# Skipped entirely if resuming, since the user has already seen this.
if (-not $global:ResumeFrom) {
    Clear-Host
    Write-Host ""
    Write-Host "  Welcome to GatewayGuard." -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  of this screen." -ForegroundColor White
    Write-Host ""
    Pause-ForUser "  Press Enter or Space once the window is maximized..."

    Write-Host ""
    Write-Host "  Now scroll to the TOP and BOTTOM of this window using the" -ForegroundColor White
    Write-Host "  scrollbar on the right side, so you don't miss anything." -ForegroundColor White
    Write-Host "  Do this on every screen going forward too." -ForegroundColor White
    Write-Host ""
    Pause-ForUser "  Press Enter or Space once you've checked top and bottom..."
}

# 3-5. Computer / domain / admin checks (always re-verified, even on resume --
# cheap checks, and confirms nothing changed about the machine mid-process)
...
# 13. Power check and sleep prevention
Test-PowerStatus
...
Show-ModeSelector
do {
    $choice = Read-ValidKey -ValidKeys @("1","2","3") -Prompt "Enter choice (1, 2, or 3): "
